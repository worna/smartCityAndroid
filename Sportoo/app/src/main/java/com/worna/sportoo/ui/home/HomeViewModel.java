...
public class HomeViewModel extends AndroidViewModel {
    ...
    public void getNextCoursesFromWeb(String email) {
        sportooWebService.getNextCustomerCourses(email).enqueue(new Callback<List<CourseDto>>() {
            @Override
            public void onResponse(@NotNull Call<List<CourseDto>> call, @NotNull Response<List<CourseDto>> response) {
                if (response.isSuccessful()) {
                    ...
                    if(newList.size() > 3)
                        _courses.setValue(newList.subList(0,3));
                    else
